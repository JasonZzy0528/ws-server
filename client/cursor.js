...
var Cursor = function (inner) {
  ...
  */
  cursor.update = function (sel, root) {
    ...
    var cursorPosition = ['start', 'end']
    cursorPosition.forEach(function (pos) {
  ...
  cursor.brFix = function () {
    ...
    if (start.el === end.el && start.offset === end.offset) {
      if (start.el.tagName === 'BR') {
        ...
        var P = (Tree.indexOfNode(br) === 0 ? br.parentNode: br.previousSibling)
        var cursorPosition = [cursor.fixStart, cursor.fixEnd]
        cursorPosition.forEach(function (f) {
          f(P, 0)
        })
